...
*   **系统调用 (Rust)**：`tokio`, `std::process::Command` (调取 Edge/Chrome 进行后台高清打印)
...
├── src/                   # Vue 3 前端代码层
│   ├── assets/            # Tailwind v4 全局主题变量映射文件及静态资源
│   ├── components/        # 核心交互组件
│   │   ├── EditorPane.vue # 左侧: CodeMirror Markdown 编辑器容器
│   │   ├── PreviewPane.vue# 右侧: Paged.js A4 打印预览容器
│   │   └── TopNavBar.vue  # 顶部标题与导出动作栏
│   ├── stores/            # Pinia 全局状态与 Markdown 数据源存储
│   ├── App.vue            # 16:9 极简黄金比例根布局
